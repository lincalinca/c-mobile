...
export default function ReviewScreen() {
  ...
  const [merchantAbn, setMerchantAbn] = useState(initialData.financial?.merchantDetails?.abn || '');
  ...
  if (!params.data && !params.uri) {
     return (
        <View className="flex-1 justify-center items-center p-6" style={{ backgroundColor: 'transparent', paddingTop: insets.top }}>
          <Text className="text-white text-lg font-bold mb-4">No Transaction Data</Text>
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: 'transparent', paddingTop: insets.top }}>
      ...
      <View className="px-6 py-4 flex-row justify-between items-center border-b border-crescender-800">
        ...
        <Text className="text-white text-lg font-bold">Review Scanned Information</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView className="flex-1 px-6 py-4">
        ...
        <View className="bg-crescender-900/40 p-4 rounded-2xl border border-crescender-800 mb-6">
          {/* Document Type Pills */}
          <View className="mb-4">
            <Text className="text-crescender-400 text-xs mb-2">Document Type</Text>
        ...
        <View className="bg-crescender-900/40 p-4 rounded-2xl border border-crescender-800 mb-6">
          ...
          <View className="flex-row gap-4">
            ...
            <View className="flex-1">
              <Text className="text-crescender-400 text-xs mb-1">GST</Text>
              <TextInput
                className="text-white text-sm border-b border-crescender-700 py-1"
                value={tax}
                onChangeText={setTax}
            ...
            <View className="flex-1">
              ...
              <TextInput
                className="text-gold text-base font-bold border-b border-gold py-1"
                value={total}
          ...
          items.map((item, index) => (
            <View key={index} className="bg-crescender-800/20 p-4 rounded-2xl mb-4 border border-crescender-800">
              ...
              <View className="flex-row justify-between items-start mb-2">
                ...
                <Text className="text-gold font-bold text-lg">${item.totalPrice?.toFixed(2)}</Text>
              </View>
              ...
              <View className="flex-row items-center gap-2 mb-3">
                ...
                {item.discountAmount && (
                  <View className="bg-green-900/50 px-2 py-0.5 rounded">
                    <Text className="text-green-400 text-xs">-${item.discountAmount.toFixed(2)}</Text>
                  </View>
                )}
                {item.discountPercentage && (
                  <View className="bg-green-900/50 px-2 py-0.5 rounded">
                    <Text className="text-green-400 text-xs">-{item.discountPercentage}%</Text>
                  </View>
                )}
              </View>

              {/* Category Selection */}
              <Text className="text-crescender-500 text-xs mb-2">Category</Text>
        ...
        >
          {isSaving ? (
            ...
            <>
              ...
              <Text className="text-crescender-950 font-bold text-lg">SAVE TRANSACTION</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
